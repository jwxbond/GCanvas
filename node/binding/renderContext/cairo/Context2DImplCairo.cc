...
void Context2DImplCairo::SetFillRule(std::string value)
{
    ...
    if (std::strcmp(value.c_str(), "evenodd") == 0) {
        rule = CAIRO_FILL_RULE_EVEN_ODD;
    }
    cairo_set_fill_rule(_context, rule);
}
void Context2DImplCairo::doFill(bool preserve = false)
{
...
void Context2DImplCairo::doStroke(bool preserve = false)
{
    cairo_pattern_t *new_pattern;
...
void Context2DImplCairo::SetLineDashOffset(float v)
{
    ...
    cairo_set_dash(_context, a.data(), dashes, offset);
}
std::string Context2DImplCairo::GetLineJoin()
{
    ...
    switch (join)
    {
        ...
        case CAIRO_LINE_JOIN_BEVEL: return "round";
...
void Context2DImplCairo::SetTextBaseline(std::string v)
{
    ...
    auto v = modes.find(v);
    if( v == modes.end() ) return;
    
    state->textBaseline = v->second;
}
