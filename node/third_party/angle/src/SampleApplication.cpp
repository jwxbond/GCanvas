      ...
      mOSWindow(nullptr)
{
    ...
    mEntryPointsLib.reset(angle::OpenSharedLibrary("libEGL.dylib"));
...
void SampleApplication::step(float dt, double totalTime) {
  ...
  nFrame += 1;
  if (at >= 1.0) {
    sprintf(buf, "%s: %d FPS", mName.c_str(), nFrame);
    mOSWindow->setWindowTitle(buf);
    nFrame = 0;
    at = 0;
  }
}
